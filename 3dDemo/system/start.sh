...
cd `dirname $0`
DEPLOY_DIR=`pwd`
CONF_DIR=$DEPLOY_DIR/system/conf


PIDS=`ps -f | grep java | grep "$CONF_DIR" |awk '{print $2}'`

LOGS_DIR=$DEPLOY_DIR/system/logs
if [ ! -d $LOGS_DIR ]; then
    mkdir $LOGS_DIR
fi
LOG_FILE=`date +%Y-%m-%d`
STDOUT_FILE=$LOGS_DIR/$LOG_FILE.log


LIB_DIR=$DEPLOY_DIR/system/lib
LIB_JARS=$DEPLOY_DIR/system/lib/*
...
nohup java $JAVA_OPTS $JAVA_MEM_OPTS $JAVA_DEBUG_OPTS $JAVA_JMX_OPTS -classpath $CONF_DIR:$LIB_JARS com.Application &
